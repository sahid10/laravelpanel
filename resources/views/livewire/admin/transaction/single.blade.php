<tr x-data="{ modalIsOpen : false }">
    ...
    <td class="">{{ $transaction->product->product_name }}</td>
    ...
    @if(getCrudConfig('Transaction')->delete or getCrudConfig('Transaction')->update)
        <td>
            ...
            @if(getCrudConfig('Transaction')->update && hasPermission(getRouteName().'.transaction.update', 1, 1, $transaction))
                <a href="@route(getRouteName().'.transaction.update', $transaction->id)" class="btn text-primary mt-1">
                    <i class="icon-pencil"></i>
                </a>
            @endif

            @if(getCrudConfig('Transaction')->delete && hasPermission(getRouteName().'.transaction.delete', 1, 1, $transaction))
